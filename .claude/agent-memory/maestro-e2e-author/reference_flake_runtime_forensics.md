...
- **CI preview Convex logs: retrievable, but only for ~15 minutes** (corrected 2026-09-22; the old "NOT retrievable" was a `--url` flag problem, not an access problem). Take the deployment name from the `E2E / seed` job log (`CONVEX_NAME: <name>`, and it prints `<name>.convex.site`), then `npx convex logs --deployment <name> --history 2000 --success --jsonl` as a logged-in CLI. It is read-only and needs no deploy key. The catch: `--history` caps at whatever the deployment still holds — measured ~983 entries on a busy preview, which reached back only 14 minutes, so a failure an hour old is already gone. **Pull them while the run is still finishing or not at all.** It streams after printing the history, so wrap it in `timeout`.

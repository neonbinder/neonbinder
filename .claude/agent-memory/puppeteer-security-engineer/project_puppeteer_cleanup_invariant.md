---
name: Puppeteer cleanup invariant in services/browser
...
In `services/browser`, BaseAdapter.launchPage launches a Puppeteer Browser child process (~150-200 MiB resident). The Browser handle is now tracked on `this.browser` and an idempotent `cleanup()` method closes it. Every adapter call in `src/index.ts` route handlers MUST be wrapped in `try { adapter.login(...) } finally { adapter.cleanup() }`.

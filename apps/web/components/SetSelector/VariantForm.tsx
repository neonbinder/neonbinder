      ...
      if (result.bscOptions.length > 0 && result.slOptions.length > 0) {
        ...
        const { stored, converged } = await storeReconciledUntilDone(
          storeReconciledOptions,
          {
            ...
            // links only on those. Without it the mutation infers coverage
            // from the items it was handed and never touches the empty side at
            // all; with a SKIPPED side wrongly in it, the mutation would
            // detach every child's slot on a marketplace nobody asked
            // (NEO-239).
        ...
        setMessage(
          ...
          // warning suffix.
          converged
            ? `Stored ${stored.optionsCount ?? items.length} variants (single platform)`
            : (stored.message ??
              "Some variants were not stored. Run the sync again to store the rest."),
        );
        // A detach the operator has not seen is a silent data change. Hold the
        // panel open so the notice renders; they close it themselves. Same for
        // an unfinished store — closing would take the only account of it.

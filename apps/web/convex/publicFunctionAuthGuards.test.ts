...
// ---------------------------------------------------------------------------
// NEO-237 / security review S4 — setDiscovery's admin-gated reads omit their
// audit fields, and refused writes persist nothing
// ---------------------------------------------------------------------------

async function seedManufacturerForCandidates(
  t: ReturnType<typeof convexTest>,
): Promise<Id<"selectorOptions">> {
  return t.run(async (ctx) =>
    ctx.db.insert("selectorOptions", {
...
describe("NEO-237 — setDiscovery reads never ship the setCandidates audit fields or the marketplace id", () => {
  test("getSetCandidates omits skippedByUserId, skippedAt, marketplaceId, and members' ids", async () => {
    const t = convexTest(schema, modules);
    const manufacturerId = await seedManufacturerForCandidates(t);
    await t.run(async (ctx) =>
      ctx.db.insert("setCandidates", {
        ...
        marketplaceId: "sl-1",
        label: "Chrome",
        members: [{ id: "sl-2", label: "Chrome Refractor" }],
        status: "skipped",
        skippedAt: Date.now(),
        skippedByUserId: "clerk_some_operator",
      }),
    ...
    const rows = await t
      .withIdentity(ADMIN)
      .query(api.setDiscovery.getSetCandidates, {
        manufacturerId,
        status: "skipped",
      });
    ...
    expect(rows[0].members).toEqual([{ label: "Chrome Refractor" }]);
    expect(rows[0].members[0]).not.toHaveProperty("id");
  });

  test("getSetCandidatesForYear carries the same omissions, year-wide", async () => {
    const t = convexTest(schema, modules);
    const sportId = await seedSport(t);
    ...
    const manufacturerId = await t.run(async (ctx) =>
      ctx.db.insert("selectorOptions", {
        level: "manufacturer",
        value: "Topps",
        platformData: {},
    ...
    const rows = await t
      .withIdentity(ADMIN)
      .query(api.setDiscovery.getSetCandidatesForYear, { yearId });
  ...
  test("a refused createSetFromCandidate persists nothing", async () => {
    const t = convexTest(schema, modules);
    const manufacturerId = await seedManufacturerForCandidates(t);
    ...
    await expect(
      t.mutation(api.setDiscovery.createSetFromCandidate, {
        candidateId,
        name: "Chrome",
      }),
    ).rejects.toThrow(/not authenticated/i);
    ...
    // Only the manufacturer seeded above — no set, no Base.
    expect(rows).toHaveLength(1);
    const candidate = await t.run((ctx) => ctx.db.get(candidateId));
    expect(candidate).not.toBeNull();
  });

  test("a refused skipSetCandidate persists nothing", async () => {
    const t = convexTest(schema, modules);
    const manufacturerId = await seedManufacturerForCandidates(t);
    const candidateId = await t.run(async (ctx) =>
      ctx.db.insert("setCandidates", {
    ...
    await expect(
      t.mutation(api.setDiscovery.skipSetCandidate, { candidateId }),
    ).rejects.toThrow(/not authenticated/i);

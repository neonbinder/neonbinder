...
// NEO-237 (D13) — a set + Base minted from one SportLots entry; since
// NEO-306 the write behind the SportLots review's "own set" save.
import {
  ...
  handler: async (ctx, args) => {
    ...
    for (const vt of variantTypes) {
      ...
      for (const ins of inserts) {
        ...
        // NEO-306 — a parallel OF an insert holds marketplace ids too
        // ("Make insert of…" lands SportLots links there). Without these the
        // Inserts and Parallels reconciles would re-offer an id already
        // placed one level down and land it on a second row — the NEO-305
  ...
  handler: async (ctx, args) => {
    ...
    for (let i = 0; i < options.length; i++) {
      ...
      if (outcome.kind === "matched") {
        ...
        // NEO-306 — the insert/parallel ROLE of a variant type, same rule:
        // read once from the row's `variant`-tagged BSC slot (as it stands
        // after this item's refresh) and recorded as an NB flag. ADDS ONLY:
        // never on a Base row, never over a role already there. Metadata
        // only — the BSC slots above are exactly what the sync wrote.
      ...
      //
      // NEO-306 — and a variant type's insert/parallel role, from the slot
      // this write is about to create (the allocated id plus the `variant`
      // tag decided two statements up). Never on the Base.
      const insertBase = confersBaseRole(item);
      const insertRole =
        level === "variantType"
 ...
 *
 * NEO-306 — a brand's pending `slSetReviews` doc is NOT a holding either. It
 * is what SportLots listed minus what NB covers, re-derived by the next Sync
 * Sets; a brand whose sets are all still waiting in the review has no
 * children and deletes like any empty row, its review going with it.
 *
...
export const getSelectorOptionHoldings = query({
  ...
  returns: v.object({
    ...
    /**
     * @deprecated NEO-239 — ALWAYS FALSE. There is no protected row class.
     *
     * NEO-219 set this from `refusesValueRename`, on the reasoning that
     * "Base/Insert/Parallel/Promo are load-bearing strings the sync,
     * `getBaseVariantBySet` and the BSC facet derivation all resolve by name".
     * None of that is true any more: the base is `metadata.isBase` and the BSC
     * `variant` facet comes from a tagged slot, so a variantType row's name is
     * a name. The other half of that reasoning — "the sync would just re-create
     * it" — is real but is a NOTICE, not a refusal, and `deleteSelectorOption`
     * already returns it as `syncedBack`.
     *
     * Kept in the shape so the delete affordance keeps compiling; the FE should
     * stop reading it.
     */
    protected: v.boolean(),
  }),
...
export async function deleteEmptySelectorOptionRow(
  ...
  // NEO-306 — a brand's pending SportLots review (one doc per year+brand) is
  // transient state keyed on the row, like the status rows above.
  if (row.level === "manufacturer" && row.parentId) {
    const yearId = row.parentId;
    const reviews = await ctx.db
      .query("slSetReviews")
      .withIndex("by_year_and_manufacturer", (q) =>
        q.eq("yearId", yearId).eq("manufacturerId", row._id),
      )
      .take(TRANSIENT_DELETE_CAP);
    for (const doc of reviews) await ctx.db.delete(doc._id);
  }
 ...
 * `SELECTOR_ROW_NOT_EMPTY` with the holdings, so the FE can say "Holds 3 sets
 * and 220 cards" rather than "cannot delete".
 *
 * NEO-239 removed a second refusal NEO-219 added: a `variantType` row nobody
 * had typed by hand was `SELECTOR_ROW_PROTECTED`, because its name was said to
 * be load-bearing for the sync, `getBaseVariantBySet` and the BSC facet
 * derivation. It is not, any more — the base is `metadata.isBase` and the
 * `variant` facet is a tagged slot — so an EMPTY variantType row deletes like
 * any other. That the next sync may re-create it is true and worth saying,
 * which is what `syncedBack` below is for; it is not grounds to refuse.
 *
 ...
 *
 * NEO-220 — now the SAME number as `MAX_CARD_PLAYERS`, the cap on the LINKED
 * spelling of this field, in exactly the way `MAX_PENDING_TEAM_NAMES` is the
 * same number as `MAX_CARD_TEAMS`. The two spellings say the same thing about
 * the same card, so a list of 20 names and a list of 20 ids must not get
 * different answers.
 */
...
/**
 * NEO-220 — validate a full-replacement `playerIds` write, and hand back the
 * linked players' names. The players twin of `resolveTeamOnCardIdsForWrite`
 * above, and deliberately the same shape line for line: dedupe first
...
async function resolvePlayerIdsForWrite(
  ...
  if (ids.length > MAX_CARD_PLAYERS) {
    throw new ConvexError(
...
export const addCustomCard = mutation({
  args: {
    ...
    attributes: v.optional(v.array(v.string())),
    /**
     * Player NAMES, as free text — the pre-NEO-220 shape.
     *
     * NEO-220 gave the quick-add form a real `PlayerPicker`, so the current
     * SPA sends `playerIds` below and never this. It stays accepted for an old
     * SPA bundle still in a tab mid-cutover (a Vercel deploy does not reload
     * anybody's browser), for exactly the reason and on exactly the terms
     * `teams` does, and it behaves as it always did: the names land in
     * `pendingPlayerNames`, surface as unknownPlayers on the next
     * `fetchCardChecklist` run, and `commitCardChecklist` clears the confirmed
     * ones so the dialog does not re-prompt. Remove it once no client can
     * still be running that bundle.
     *
    ...
    /**
     * NEO-220 — the players printed on this card, as real `players` ids.
     *
     * The quick-add form's `PlayerPicker` produces these, so a hand-added card
     * is born LINKED rather than born naming a player it does not point at
     * (which `deriveCardAttention` badges as an `unreviewedName`, sending the
     * walker to ask the operator for something they had just supplied).
     * Validated by `resolvePlayerIdsForWrite`, the players twin of the helper
     * `teamOnCardIds` uses.
     */
    playerIds: v.optional(v.array(v.id("players"))),
    /**
  ...
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const typedPlayerNames = normalizePendingNames(
      args.players,
      "player",
    ...
    // NEO-220 — validated BEFORE anything is written, same as the teams below,
    ...
    // two never coexist on a row this mutation writes — mirroring the teams
    // rule directly below, and keeping a row from being badged as carrying an
    // unreviewed name it has in fact already linked. Nothing sends both today
    ...
    // never regenerated automatically once a row exists).
    //
    // NEO-220: the title says the LINKED players' stored names when the
    // quick-add form's PlayerPicker sent ids. The pending-name fallback stays
    // underneath it for an old SPA bundle that sent `players: [string]`: those
    // names aren't resolved to `players` rows yet (that happens later, if
...
export const setVariantTypePlatformData = mutation({
  args: {
    ...
    // picker, and both writes below REUSE an existing slot key — the SportLots
    // primary via `setPrimarySlotId`, and (NEO-239) the BSC setName-tagged slot
    // in place. A blind write would silently re-point every card already on
    // that slot at whatever the other session just mapped. Refuse instead and
    // let the operator re-read.
...
/**
 * Internal: drain `slSetReviews` (NEO-306), looped by `runSetBuilderReset`
 * before `selectorOptions`, whose year and brand rows each doc names.
 */
export const resetSlSetReviewsBatch = internalMutation({
  args: {},
  returns: v.object({
    deleted: v.number(),
    hasMore: v.boolean(),
  ...
  handler: async (ctx) => {
    assertResetArmed();
    const rows = await ctx.db.query("slSetReviews").take(RESET_BATCH_SIZE);
    for (const row of rows) {
      await ctx.db.delete(row._id);
    }
    return { deleted: rows.length, hasMore: rows.length === RESET_BATCH_SIZE };
...
/**
 * NEO-237 (D13) — SportLots-only names becoming NB sets: a `setName` row
 * with no marketplace ids and a "Base" `variantType` child carrying the
 * SportLots id (`insertSetWithBaseFromSl`), the way the BSC phase stores
 * BSC's sets. Jason, 2026-09-21: "If a set exists in a marketplace it should
 * be saved whether it is in SL or BSC or both."
 *
 * NEO-306 — no longer called by Sync Sets. The sync writes the names into
 * the brand's review (`slSetReviews`), and the review's save calls
 * `createSetsFromSlRootsImpl` for the names the operator left as their own
 * set, in the same transaction that removes them from the review.
 *
 ...
 * retry mid-way for the same reason. `createdByUserId` is the calling
 * admin's identity, threaded from the action — never a client argument.
 *
 * ## The read and write budget
 *
 * One transaction takes at most `MAX_SL_SETS_PER_MUTATION` roots (each is
 * two inserts and two patches); the caller chunks a longer list
 * (`chunkSlRoots`) and sums the counts, so a full review
 * (`MAX_SL_SETS_PER_SYNC` entries) is five transactions, not one. The duplicate checks read the year's `setName` rows ONCE per
 ...
 */
export async function createSetsFromSlRootsImpl(
  ctx: MutationCtx,
  args: {
 ...
 * a set NB already has year-wide, or a SportLots-only NAME. NEO-306: every
 * such name becomes one entry in that brand's review (`slSetReviews`,
 * `convex/slSetReview.ts`, replaced per brand scope on every successful
 * list), where the operator files it as its own set or as a row under one of
 * the brand's BSC-linked sets' variant types; nothing is minted here. (NEO-237
 * minted every root as a set, and NEO-305 parked the flagship's colours
 * nowhere; the review replaces both.) Brands linked THROUGH SportLots' all-brands
...
export const syncSetsAcrossManufacturers = action({
  ...
  returns: v.object({
     ...
     * NEO-305's flagship absorb are both gone); the operator files each name
     * in the review. Telemetry only: the column's pill reads the review doc.
     */
    slPendingReview: v.number(),
  }),
  handler: async (
    ctx,
    args,
    ...
    AggregatedSyncResult & {
      totalSets: number;
      slPendingReview: number;
    }
  > => {
    // NEO-306 — the SportLots phase mints nothing, so no identity is
    // threaded anywhere; the review's save records its own caller.
    await requireAdmin(ctx);
    const requestId = newRequestId();
    try {
      ...
      const unlinkedAll: UnlinkedEntry[] = [];
      let totalStored = 0;
      // NEO-306 — SportLots-only names written into the brands' reviews.
      let slPendingReviewTotal = 0;
      ...
      if (allBrandsScopes.length === 0 && realScopes.length === 0) {
        ...
        let slFailed = 0;
        // NEO-306 — SportLots-only names written into the brands' reviews
        // this run (`slSetReviews`), summed across scopes. Nothing is minted
        // here any more: the operator files each name in the review.
        let slPendingReview = 0;
        // Names past the per-sync cap: not lost, they are the first thing
        ...
        /**
         * NEO-306 — classify one brand scope's SportLots list and REPLACE its
         * review doc with what is left. Called only for a scope whose list
         * came back ok; a scope whose covered-id read was truncated keeps its
         * previous doc (a truncated covered set would re-offer names NB
         * already links). The NEO-294 known-brand split for the Unknown
         * scope is a placement, applied by the review's save to the names
         * filed as sets (`applySlSetReview`), not here.
         */
        const classify = async (
          ...
          if (routed.truncated > 0) {
            console.warn(
              `[syncSetsAcrossManufacturers] SportLots list truncated for one ` +
                `brand (entries +${routed.truncated})`,
            );
          }
          ...
          slPendingReviewTotal += routed.entries.length;
        };
        ...
        // NEO-306 — draft copy (accepted for now; Jason reviews on local
        // Vite). Log-only: `message` never reaches a surface (the column's
        // pill reads the review doc itself).
        summary.push(`${countNoun(slPendingReview, "SportLots set")} to sort`);
        if (slRootsTruncated > 0) {
          summary.push(
            `${countNoun(slRootsTruncated, "more SportLots set")} to sort ` +
              `on the next sync`,
          );
        }
        if (slUnnameable > 0) {
...
export const commitCardChecklistPrelude = internalMutation({
  ...
  returns: v.object({
    ...
    //
    // OVERLAPS `unresolvedTeamNames` above without duplicating it, and the two
    // are kept apart because they answer different questions. This one is
    // "nobody ruled on it", for both kinds, and drives what the operator SEES
    // (pending names on the card, the banner count). That one is "no team id
    // came out of it", team-only, and includes a reviewed `create` decision
    // that carried no operator Location + Name — a ruling that still produced
    // no row. Collapsing them would either stamp a pending name on a card the
    // operator did rule on, or drop a create-without-a-pair out of the log.
  ...
  handler: async (ctx, args): Promise<CommitPrelude> => {
    ...
    // NEO-253: the SAME function `players` dedupes on, not a copy of it.
    //
    // NEO-236: NOT a team-identity normaliser any more. Team lookups go
    // through `findTeamByFullName` (convex/lib/teamRow.ts), which owns the one
    // derivation of a team's dedup key; this is used for the PLAYERS index
    // lookup and for keying `reviewByKey`/`excludedCareerTeamNames`/
    // `careerTeamCreateBySource`, which are name-keyed maps built and read
    // entirely inside this file and so only have to agree with themselves.
    // Do not reach for it to find a `teams` row.
    //
    // It is still the SHARED function rather than a transcription, and that is
    // load-bearing on both counts. The players lookup hits
    // `by_name_normalized_and_sport_id`, so a copy that drifted by one
    // character would miss rows the table can see; and the local maps are read
    // against keys minted elsewhere in the same transaction, so a copy that
    // stopped folding would key "José Ramírez" and "Jose Ramirez" apart while
    // the row they both resolve to is one. This was a hand copy until NEO-253,
    // and it was the copy that still shredded accented names after `nameKey`
    // had learned to fold them.
    ...
    const createTeamFromOperatorInput = async (
      ...
      opts: {
         ...
         * `extra.leagueId`, because the answer that needs protecting most is
         * "no league": that produces NO `leagueId` at all, which is exactly
         * what an unanswered decision produces too. Without the flag the two
         * are indistinguishable and the default silently wins — which is the
         * defect Jason found ("How does this dialog know which League the new
    ...
    // NEO-236: declared here rather than beside the team loop, because the
    // staged-career-team pass below runs BEFORE that loop and pushes into it.
    const createdTeamIds: Array<Id<"teams">> = [];

    /**
     * NEO-236 — the stored fields a REVIEWED team row is built from, beyond its
     * Location and Name.
     *
     * Shared by the two places a create decision becomes a `teams` row — the
     * staged career-team pass and the checklist team loop — so the two cannot
     * drift on what a reviewed team carries or on which source wins.
     *
     * The League is the part this ticket changed, and the order is the fix:
     *
     *  1. **What the operator chose.** An id, or `null` for "no league", or a
     *     name for a league we do not hold yet. Answered means answered — the
     *     sport default is never allowed back over it, which is what stopped a
     *     club side pulled off a player's career list from being filed under
    ...
    const reviewedTeamFields = async (
      ...
      if (create.leagueId !== undefined && storedLeagueUsable) {
        ...
        // they made on the New Team step, and the step's pre-fill is only
        // where that decision started. Ordered above `enrichment.league`
    ...
    /**
     * ── NEO-236: staged career teams are created BEFORE anything needs them ──
     *
     * These rows are not in `allTeamNames` — no card carries their name. They
     * were staged by `entityReviewQueue.stageCareerTeamRows` off a player's own
     * career list, walked as their own New Team step, and answered with a
     * Location, a Name and a League. This pass is where those answers become
     * `teams` rows.
     *
     * It runs FIRST, ahead of both name loops, and the ordering is the whole
     * mechanism: the player loop below resolves each stint by looking the
     * career-team label up with `findTeamByFullName`, so a team created here is
     * simply found there. Nothing has to hand an id across — the batch's own
     * dedup key does it, which is also why an operator answering "Padres" with
     * ("San Diego", "Padres") lands on the row we already had instead of a
     * second one.
     *
     * A staged row the operator SKIPPED or never answered creates nothing and
     * is not reported as unresolved here: it is not a name off the checklist,
     * so there is no card sitting on it, and the player loop reports the stint
     * label itself if it still cannot be resolved.
     */
    /**
     * NEO-236 — the label a staged step ANSWERED, to the team it produced.
     *
     * Keyed by the label the step was raised for, and consulted before any
     * name lookup, because the operator is allowed to answer "Sydney Blue Sox"
     * with something that is not spelled that way. They might split it
     * ("Sydney" / "Blue Sox", which still composes back and would match), and
     * they might not ("Sydney Blue Sox Baseball Club", which would not) — and
     * they might answer it by LINKING to a row we already hold under a
     * different name, which no amount of matching on the label could find.
     *
     * Resolving the stint through this map instead of through the label is the
    ...
    for (const row of reviewRows) {
      ...
      if (row.decision?.action !== "create") continue;
      const create = row.decision.create;
      // No Location + Name means the step was never filled in — nothing to
      // build a row from, and this path has no fallback to the raw label by
    ...
    for (
      ...
      // NEO-236: team IDENTITY goes through the shared lookup in
      // convex/lib/teamRow.ts. This used to be a hand-rolled
      // `by_name_normalized_and_sport_id` query keyed on `norm` above — a
      // third copy of `normalizeTeamName` sitting next to the two in
      // teams.ts/entityNearMatch.ts. All three are byte-identical today, which
      // is exactly why the drift would be silent: the day one of them gains a
      ...
      // unreviewed case counts as either "unresolved" (NEO-236) or
      // "unreviewed" (NEO-221): a skip is an answer to both questions.
      if (!decision || decision.action === "skip") {
        if (!decision) {
          unresolvedTeamNames.add(name);
          // NEO-221 — same rule as the player loop above.
          unreviewedTeamNames.push(name);
        }
        continue;
      }
...
type CommitPrelude = {
  ...
  /** NEO-221 — see the returns validator. */

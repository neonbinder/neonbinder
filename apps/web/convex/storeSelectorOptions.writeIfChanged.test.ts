...
describe("storeSelectorOptions write-if-changed (NEO-85)", () => {
  ...
  test("DOES patch when only setMetadata differs (platformData unchanged)", async () => {
    const t = convexTest(schema, modules);
    const asAdmin = t.withIdentity(ADMIN_IDENTITY);
    ...
    const id: Id<"selectorOptions"> = await t.run(async (ctx) =>
      ctx.db.insert("selectorOptions", {
        level: "setName",
        value: "2022 Topps",
        platformData: { bsc: "bsc-set" },
        setMetadata: { block: "Series 1" },
        children: [],
        lastUpdated: SENTINEL,
      }),
    );
    ...
    expect(row?.lastUpdated).not.toBe(SENTINEL);
    expect(row?.lastUpdated).toBeGreaterThan(SENTINEL);
    expect(row?.setMetadata?.block).toBe("Series 2");
  });

  test("does NOT patch when supplied setMetadata merges to identical data", async () => {
    const t = convexTest(schema, modules);
    const asAdmin = t.withIdentity(ADMIN_IDENTITY);

"""Unit tests for app.dhash.

Covers: hash width/determinism, `hamming_distance`, the
`SAME_IMAGE_THRESHOLD` boundary (parametrized off the production constant),
and the separation the Convex pairing consumer actually depends on — two
encodings/crops of the same scan landing inside the threshold while a
genuinely different image lands far outside it.
...
class TestSameImageVersusDifferentImage:
    """The one decision the Convex pairing consumer uses this for:
    re-scan or mis-classification?"""

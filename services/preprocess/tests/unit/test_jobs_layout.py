...
fixed-width zero padding the per-entry names carry, and the image-key
derivation that makes zip slip structurally impossible. There are no status
or manifest keys to cover: Convex owns all job state and results (NEO-170).
"""
...
class TestKeys:
    def test_every_key_sits_under_the_job_prefix(self):
        ...
        for key in (
            ...
            layout.extracted_image_object(USER, JOB, 0, "jpg"),
    ...
    def test_no_status_or_manifest_keys_exist(self):
        # Convex owns all job state and results (NEO-170). A status log or a
        # manifest reappearing here would be a second, divergent source of
        # truth — nothing may recreate them.
        for name in ("status_prefix", "status_object", "manifest_object", "MANIFEST_OBJECT"):
            assert not hasattr(layout, name)


class TestExtractedImageObject:
    def test_key_shape(self):
        assert layout.extracted_image_object(USER, JOB, 7, "jpg") == (
            f"placeholders/{USER}/{JOB}/extracted/0007.jpg"
        )

    @pytest.mark.parametrize("extension", ["jpg", ".JPG", "PNG", "webp"])
    def test_extension_is_normalised(self, extension):
        key = layout.extracted_image_object(USER, JOB, 7, extension)
        assert key.endswith(f"0007.{extension.lower().lstrip('.')}")

    def test_index_is_zero_padded(self):
        assert layout.extracted_image_object(USER, JOB, 3, "jpg").endswith("extracted/0003.jpg")

    def test_negative_index_rejected(self):
        with pytest.raises(InvalidJobIdentifierError):
            layout.extracted_image_object(USER, JOB, -1, "jpg")

    @pytest.mark.parametrize("extension", ["../../etc/passwd", "jpg/../..", "j g", ""])
    def test_non_alphanumeric_extension_rejected(self, extension):
        with pytest.raises(InvalidJobIdentifierError):
            layout.extracted_image_object(USER, JOB, 0, extension)

    @pytest.mark.parametrize("payload", TRAVERSAL_PAYLOADS)
    def test_rejects_traversal_identifiers(self, payload):
        with pytest.raises(InvalidJobIdentifierError):
            layout.extracted_image_object(payload, JOB, 0, "jpg")
